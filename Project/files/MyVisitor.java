...
public class MyVisitor extends DepthFirstAdapter
{
	...
	public void inADefFunction(ADefFunction node)
	{
		...
		*/
		if(functionCallStack.size() != 0) return;
	...
	public void outADefFunction(ADefFunction node)
	{
		/*
			...
			allFuctionWithSameName : Hashtable<String, Function>
	...
	public void inAAfterFirstArgNextArgs(AAfterFirstArgNextArgs node)
	{
		...
		*/
		...
		if(defaultValueOfNextArgs == 0 && fData.getDefaultArgs() > 0)
			print("In line " + ((AIdentifierExpression)node.getId()).getId().getLine() + " : " +
			"'" + ((AIdentifierExpression)node.getId()).getId().getText() + 
			"'\nErrorMessage : Default arguments can't be followed by non default arguments");
		...
		if(symtable.containsKey(fCallname))
		{
			for(Function function : ((Hashtable<String, Function>)symtable.get(fCallname)).values())
			{
				if(args.size() <= function.getArgs() && args.size() >= (function.getArgs() - function.getDefaultArgs()))
				{
					...
					if(currentFuction == "")
					{
						functionCallStack.push(fCallname);
						caseADefFunction(function.getADefFunction());
